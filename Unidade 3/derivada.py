def Regra_Extremidade_Esquerda (a = 0, b = 0, c= 0, d = -2, e = 0, f = 2,
  ...
  if plot == True:
    ...
    plt.text(0.3, 0.15,
             rf'$\int_{{{x_i}}}^{{{x_f}}} f(x)\,dx=\int_{{{x_i}}}^{{{x_f}}} {a}x^5 + {b}x^4 + {c}x^3 + {d}x^2 + {e}x + {f}\,dx = {somatoria:.2f}$',
    ...
    plt.ylabel(r'$f(x)$')
    plt.legend()
    plt.savefig('integracao_extremidade_esquerda.png', dpi = 1200)
    plt.show()
